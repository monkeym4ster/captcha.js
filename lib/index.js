...
class Captcha {
  ...
  randomChars() {
    ...
    let chars = _.sampleSize(_chars, this.length || this.opt.length).join('');
  ...
  create(callback) {
    this.code = this.randomChars();

    const pngFilePath = `${path.join(this.opt.cache_dir, this.code)}.png`;

    this.file_path = pngFilePath;
...
module.exports = (length, opt) => new Captcha(length, opt);
